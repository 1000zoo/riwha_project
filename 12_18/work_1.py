...
for i in range(1, len(init_lat)):
    nlat = abs(center_lat[i].value - side_lat[i].value) / 6 + center_lat[i].value
    nlng = abs(center_lng[i].value - side_lng[i].value) / 6 + center_lng[i].value
    ...
    if lane_type[i].value == "lane1" and edist > cen_min[i].value:
